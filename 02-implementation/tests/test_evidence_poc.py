...
from pipeline.evidence import classify_edge, temporal_check
...
def test_first_latency_anomaly_row() -> None:
    import pandas as pd

    from pipeline.reality import first_latency_anomaly_row

    empty = pd.DataFrame()
    normal = pd.DataFrame(
        [{"service_name": "svc", "duration": d} for d in [10, 11, 12, 10, 11] * 20]
    )
    abnormal = pd.DataFrame(
        [
            {"time": "2025-01-01T00:00:01Z", "service_name": "svc", "duration": 12},
            {"time": "2025-01-01T00:00:02Z", "service_name": "svc", "duration": 500},
            {"time": "2025-01-01T00:00:03Z", "service_name": "svc", "duration": 600},
        ]
    )

    assert first_latency_anomaly_row(empty, normal, "svc", 2.0) is None
    assert first_latency_anomaly_row(abnormal, empty, "svc", 2.0) is None
    assert first_latency_anomaly_row(abnormal, normal, "other_svc", 2.0) is None
    assert first_latency_anomaly_row(abnormal, normal, "svc", 100.0) is None

    row = first_latency_anomaly_row(abnormal, normal, "svc", 2.0)
    assert row is not None
    assert row["time"] == "2025-01-01T00:00:02Z"
    assert row["duration"] == 500


def _traces_frame(rows: list[dict]):
    import pandas as pd

    return pd.DataFrame(rows)


def test_temporal_check_falls_back_to_latency_anomaly() -> None:
    rules = load_rules()
    normal = _traces_frame(
        [{"service_name": "up", "duration": 10}] * 20
        + [{"service_name": "down", "duration": 10}] * 20
    )
    abnormal = _traces_frame(
        [
            {
    ...
    reality = {"_frames": {"abnormal": abnormal, "normal": normal}}
    registry: list[dict] = []
    status, ids = temporal_check("up", "down", reality, registry, rules)
    assert status == "pass"
    assert ids
    claim = registry[-1]["claim"]
    assert "latency-anomaly onset" in claim
    assert "up" in claim and "down" in claim


def test_temporal_check_still_unknown_with_no_signal() -> None:
    rules = load_rules()
    normal = _traces_frame(
        [{"service_name": "up", "duration": 10}] * 20
        + [{"service_name": "down", "duration": 10}] * 20
    ...
    reality = {"_frames": {"abnormal": abnormal, "normal": normal}}
    registry: list[dict] = []
    status, _ = temporal_check("up", "down", reality, registry, rules)
    assert status == "unknown"
    assert registry[-1]["claim"] == (
        "No Error span and no latency anomaly at one or both endpoints; "
        "onset order not determinable"
    )


def test_algo_breakdown_table_excludes_auto_sentinel() -> None:
    cases = [
        {
            "algo_name": "auto",
            "algo_hit_at_1": None,
            "rca_status": "insufficient_evidence",
            "rca_reason": "algo_output_missing",
        },
        {
            "algo_name": "baro",
            "algo_hit_at_1": True,
            "rca_status": "candidate_path_constructed",
            "rca_reason": None,
        },
    ]
    table = _algo_breakdown_table(cases)
    assert "<th>auto</th>" not in table
    assert "<th>baro</th>" in table
...
def test_call_pair_stats_and_timeline_honesty() -> None:
    ...
    from adapters.trace_graph import call_pair_stats
...
def test_aggregate_agreement_and_index_rq() -> None:
    ...
    assert "80.0%" in page2  # sibling's path_coverage (0.8), shown as a percentage not a raw fraction
