...
from .algo_context import AUTO_ALGO
...
REFUSE_PLAIN = {
    "no_connected_candidate_path": "no valid connection",
    "required_horizontal_relationship_unavailable": "no connected route",
    "required_vertical_topology_unavailable": "no connected route",
    "algo_output_missing": "algorithm output missing",
    "ambiguous_equally_supported_paths": "ambiguous, multiple equally-supported paths",
    "injected_component_unavailable": "injected component unavailable",
    "symptom_unavailable": "symptom unavailable",
    "evidence_query_failed": "evidence query failed",
    "unsupported_case_schema": "unsupported case schema",
}


def refuse_plain(reason: str | None) -> str:
    if not reason:
        return "no reason recorded"
    return REFUSE_PLAIN.get(reason, reason.replace("_", " "))
...
def _refuse_count_rows(counts: dict) -> str:
    ...
    total_refused = (counts or {}).get("refused") or sum(reasons.values())
    return "".join(
        f"<tr><td>{html.escape(refuse_short(r))}</td><td>{_pct(n / total_refused)}</td></tr>"
        for r, n in sorted(reasons.items(), key=lambda kv: -kv[1])
    )


def outcome_breakdown_rows(cases: list[dict]) -> list[tuple[str, str, list[dict]]]:
    """Fixed row structure shared by every outcome-breakdown table (algorithm columns,
    AI-model columns, or any future metric): (RCA verdict, Path verdict, cases-in-row),
    biggest group first. Path-refused rows are split one per distinct `rca_reason`
    actually present - never hardcoded to a fixed count of reasons."""

    def reason_groups(subset: list[dict]) -> list[tuple[str | None, list[dict]]]:
        by_reason: dict[str | None, list[dict]] = {}
        for c in subset:
            by_reason.setdefault(c.get("rca_reason"), []).append(c)
        return sorted(by_reason.items(), key=lambda kv: -len(kv[1]))

    correct = [c for c in cases if c.get("algo_hit_at_1") is True]
    wrong = [c for c in cases if c.get("algo_hit_at_1") is False]
    unavailable = [c for c in cases if c.get("algo_hit_at_1") is None]
    correct_built = [c for c in correct if c.get("rca_status") == "candidate_path_constructed"]
    correct_refused = [c for c in correct if c.get("rca_status") != "candidate_path_constructed"]
    wrong_built = [c for c in wrong if c.get("rca_status") == "candidate_path_constructed"]
    wrong_refused = [c for c in wrong if c.get("rca_status") != "candidate_path_constructed"]

    rows: list[tuple[str, str, list[dict]]] = [
        ("RCA CORRECT", "Path correct / built", correct_built),
    ]
    for reason, group in reason_groups(correct_refused):
        rows.append(("RCA CORRECT", f"Path refused — {refuse_plain(reason)}", group))
    rows.append(("SUBTOTAL — RCA CORRECT", "", correct))
    rows.append(("SPACER", "", []))
    rows.append(("RCA WRONG", "Path built", wrong_built))
    for reason, group in reason_groups(wrong_refused):
        rows.append(("RCA WRONG", f"Path refused — {refuse_plain(reason)}", group))
    rows.append(("SUBTOTAL — RCA WRONG", "", wrong))
    rows.append(("SPACER", "", []))
    if unavailable:
        rows.append(("UNAVAILABLE", "Algorithm unavailable", unavailable))
    rows.append(("TOTAL", "", cases))
    return rows


def _algo_breakdown_table(cases: list[dict]) -> str:
    """Outcome breakdown x baseline-algorithm columns. Columns are whatever `algo_name`
    values are actually present in `cases` - one baseline today, ready for more."""
    total = len(cases)
    if not total:
        return "<p class='muted'>no cases</p>"

    algos = sorted(
        {c.get("algo_name") for c in cases if c.get("algo_name") and c.get("algo_name") != AUTO_ALGO}
    )

    def algo_cells(subset: list[dict], *, show: bool) -> str:
        n = len(subset)
        if not show:
            return "<td></td>" * len(algos)
        if not n:
            return "<td>—</td>" * len(algos)
        cells = []
        for algo in algos:
            count = sum(1 for c in subset if c.get("algo_name") == algo)
            cells.append(f"<td>{count} ({_pct(count / n)})</td>" if count else "<td>—</td>")
        return "".join(cells)

    header_algo = "".join(f"<th>{html.escape(a)}</th>" for a in algos)
    body = []
    for verdict, path_verdict, subset in outcome_breakdown_rows(cases):
        if verdict == "SPACER":
            body.append(f"<tr class='spacer'><td colspan='{4 + len(algos)}'></td></tr>")
            continue
        n = len(subset)
        if verdict.startswith("SUBTOTAL") or verdict == "TOTAL":
            cells = algo_cells(subset, show=verdict != "TOTAL")
            body.append(
                f"<tr class='subtotal'><td colspan='2'>{html.escape(verdict)}</td>"
                f"<td>{n}</td><td>{_pct(n / total)}</td>{cells}</tr>"
            )
            continue
        body.append(
            f"<tr><td>{html.escape(verdict)}</td><td>{html.escape(path_verdict)}</td>"
            f"<td>{n}</td><td>{_pct(n / total)}</td>{algo_cells(subset, show=True)}</tr>"
        )

    return f"""<table class='breakdown'>
<thead><tr><th>RCA Verdict</th><th>Path Verdict</th><th>Cases (n)</th><th>% of Total</th>{header_algo}</tr></thead>
...
def _strat_dual_table(seed_tables: dict, *, title: str) -> str:
    ...
    for key in keys:
        ...
        n = i["n"] or r["n"]
        i_pct = _pct(i["accept"] / i["n"]) if i["n"] else "—"
        r_pct = _pct(r["accept"] / r["n"]) if r["n"] else "—"
        rows.append(
            "<tr>"
            f"<td>{html.escape(key)}</td><td>{n}</td>"
            f"<td>{i_pct}</td><td>{r_pct}</td>"
            "</tr>"
        )
    body = "".join(rows)
    return f"""
<table>
<thead><tr>
<th>{html.escape(title)}</th><th>n</th>
<th>Inject accept</th><th>RCA accept</th>
</tr></thead>
<tbody>{body}</tbody>
</table>
"""
...
def _rqa_coverage_table(summary: dict, *, sample: str, policy: str) -> str:
    ...
    return f"""
<table>
<thead><tr><th>Policy</th><th>n</th><th>Inject cov</th><th>RCA cov</th></tr></thead>
<tbody>{body}</tbody>
</table>
{note}
"""
...
def index_html(summary: dict, *, sample: str, policy: str, rules: dict | None = None) -> str:
    ...
    breakdown_table = _algo_breakdown_table(summary.get("cases") or [])

    max_hops = (rules.get("path_search") or {}).get("max_hops", "?")
    max_inferred = ((rules.get("acceptance_presets") or {}).get("relaxed") or {}).get(
        "max_inferred_edges", "?"
    )
    lead = (
        "Each candidate path is a chain of edges between services (nodes). Every edge is "
        "checked both horizontally (does the evidence connect these two services at all) "
        "and vertically (pod/node/container placement), then labelled Observed, Supported, "
        "or Inferred. <strong>strict</strong> accepts only Observed/Supported edges; "
        f"<strong>relaxed</strong> allows up to {max_inferred} Inferred edges, both within "
        f"{max_hops} hops. A path failing that bar is refused, not guessed. "
        "Click any case below for its full evidence trail."
    )

    agree_total = len(cases) or 1
    agree_tiles = "".join(
        f"<div class='tile'><div class='n'>{_pct(profile.get(k, 0) / agree_total)}</div>"
...
<table><thead><tr><th>Reason</th><th>% of refusals</th></tr></thead>
