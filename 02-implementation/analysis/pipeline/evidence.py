...
from .reality import first_latency_anomaly_row, first_onset_row
...
def temporal_check(
    ...
    """Compare first-onset timestamps at src and dst; unknown when either is missing
    or the two onsets fall within tolerance of each other (can't order them)."""
    eid = f"temp_{src}__{dst}"
    temporal_rules = (rules.get("checks") or {}).get("temporal") or {}
    tolerance = float(temporal_rules.get("onset_tolerance_seconds", 1.0))
    latency_factor = float(temporal_rules.get("latency_anomaly_factor", 2.0))

    frames = reality.get("_frames") or {}
    abn_frame = frames.get("abnormal")
    norm_frame = frames.get("normal")
    src_row = first_onset_row(abn_frame, src, require_error=True) if abn_frame is not None else None
    dst_row = first_onset_row(abn_frame, dst, require_error=True) if abn_frame is not None else None
    signal = "Error onset"

    if src_row is None and dst_row is None and abn_frame is not None and norm_frame is not None:
        src_row = first_latency_anomaly_row(abn_frame, norm_frame, src, latency_factor)
        dst_row = first_latency_anomaly_row(abn_frame, norm_frame, dst, latency_factor)
        signal = "latency-anomaly onset"

    status = "unknown"
    delta = None
    claim = "No Error span and no latency anomaly at one or both endpoints; onset order not determinable"
    if src_row is not None and dst_row is not None:
        delta = (pd.Timestamp(dst_row["time"]) - pd.Timestamp(src_row["time"])).total_seconds()
        if delta > tolerance:
            status = "pass"
            claim = f"{src} {signal} precedes {dst} {signal} by {delta:.3f}s"
        elif delta < -tolerance:
            status = "fail"
            claim = f"{dst} {signal} precedes {src} {signal} by {-delta:.3f}s (reversed order)"
        else:
            claim = f"{signal.capitalize()}s within {tolerance:.3f}s of each other ({delta:+.3f}s); cannot order"
    ...
    registry.append(
        {
            ...
            "row_count": int(src_row is not None) + int(dst_row is not None),
            "result_hash": _hash_payload(
                {"src": src, "dst": dst, "status": status, "delta_s": delta}
            ),
            "claim": claim,
        }
    )
...
def annotate_paths(
    ...
    def ensure_edge(src: str, dst: str) -> dict:
        ...
        if src_stat == "pass" and dst_stat == "pass":
            ...
            statistical = "fail"
        temporal, trefs = temporal_check(src, dst, reality, registry, rules)
        checks = {
            "structural": struct,
