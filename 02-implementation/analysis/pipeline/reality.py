...
from adapters.ground_truth import injected_targets
...
def first_onset_row(
    ...
    *,
    ...
    """Earliest Error span for `service` in the abnormal window, else earliest span.

    `require_error=True` returns None instead of falling back to "earliest span" -
    that fallback is just the first request in the window (near-simultaneous across
    every service) and carries no signal about when a service actually went bad;
    ordering two of those against each other would be noise, not evidence.
    """
    ...
    if require_error or scoped.empty:
        return None
    return scoped.sort_values("time").iloc[0]


def first_latency_anomaly_row(
    abnormal_traces: pd.DataFrame,
    normal_traces: pd.DataFrame,
    service: str,
    factor: float,
):
    """Earliest abnormal-window row for `service` whose duration exceeds
    `factor` times that service's 95th-percentile duration in the normal window.

    Fallback signal for fault classes (pure latency/delay) that never produce an
    Error-status span, so `first_onset_row(require_error=True)` can never resolve
    for them.
    """
    if abnormal_traces.empty or normal_traces.empty:
        return None
    normal_scoped = normal_traces[normal_traces["service_name"] == service]
    if normal_scoped.empty or "duration" not in normal_scoped.columns:
        return None
    baseline = normal_scoped["duration"].dropna().quantile(0.95)
    scoped = abnormal_traces[abnormal_traces["service_name"] == service]
    if "duration" not in scoped.columns:
        return None
    anomalous = scoped[scoped["duration"] > factor * baseline]
    if anomalous.empty:
        return None
    return anomalous.sort_values("time").iloc[0]
...
def build_timeline(
    ...
    first = first_onset_row(abnormal_traces)
    ...
    symptom_row = first_onset_row(abnormal_traces, symptom["component"])
...
def enrich_timeline_path_errors(
    ...
    for node in path_nodes:
        ...
        if key in existing or ("first_error_span", node) in existing:
            continue
        row = first_onset_row(abnormal_traces, node)
