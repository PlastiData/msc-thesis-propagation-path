#!/usr/bin/env python3
"""Evidence-backed failure-propagation POC entry point.
...
from evidence.algo_context import AUTO_ALGO, DEFAULT_ALGO, load_algo_context, load_dataset_perf
...
from evidence.evidence import annotate_paths
from evidence.graph import build_candidate_graph, paths_between
from evidence.judgment import case_metrics, judge
from evidence.reality import build_reality, enrich_timeline_path_errors
...
def _print_banner(
    *,
    ...
    print("evidence_path_poc", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--sample",
        type=Path,
        default=ANALYSIS_DIR / "sample10.txt",
        help="case list file (default: analysis/sample10.txt)",
    )
    parser.add_argument(
        "--case",
        action="append",
